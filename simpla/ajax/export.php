...
{	
	private $columns_names = array(
			...
			'variant'=>          	'Вариант',
			'sku'=>              	'Артикул',
			'package_weight'=>		'Вес в пачке',
			'minimum_amount'=>		'Мин. заказ',
			'items_per_package'=>	'Кратно',
			'stock'=>            	'Склад',
		...
		foreach($variants as $variant)
 		{
 			if(isset($products[$variant->product_id]))
 			{
				...
				$v['package_weight']  	= $variant->package_weight;
				$v['minimum_amount']  	= $variant->minimum_amount;
				$v['items_per_package'] = $variant->items_per_package;
	 			if($variant->infinity)
	 				$v['stock']         = '';
